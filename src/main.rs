...
use multitap::{Matrix, MultiTap, MultiTapEvent};
...
async fn main(_spawner: Spawner) {
    ...
    let mut buffer: [Option<MultiTapEvent>; 80] = [Default::default(); 80];

    let t_style = MonoTextStyleBuilder::new()
        .font(&FONT_6X10)
    ...
    let mut text_input = TextInput::new(&mut model, style, t_style);

    loop {
        text_input.update(multi_tap.event().await);
