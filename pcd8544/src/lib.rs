    ...
    RST: OutputPin<Error = PinE>,
{
    pub fn new(mut display_interface: DI, reset: RST) -> Self {
        Self {
            display_interface,
            reset,
            buffer: [0x00; LCDWIDTH * LCDHEIGHT / 8],
        }
    }
    ...
    pub fn init(&mut self, delay_source: &mut impl DelayNs) {
        ...
        self.display_interface
            ...
            .unwrap();
        self.display_interface
            .send_commands(DataFormat::U8(&[PCD8544_SETBIAS | 0x04]))
            .unwrap();
        self.display_interface
            .send_commands(DataFormat::U8(&[PCD8544_FUNCTIONSET]))
            .unwrap();
        self.display_interface
            .send_commands(DataFormat::U8(&[
                PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION
            ...
            .unwrap();
        self.display_interface
            .send_commands(DataFormat::U8(&[PCD8544_SETVOP | 75]))
            .unwrap();
        self.display_interface
            .send_commands(DataFormat::U8(&[PCD8544_FUNCTIONSET]))
            .unwrap();
    ...
    pub fn flush(&mut self) {
        for page in 0..(LCDHEIGHT / 8) {
            self.display_interface
                .send_commands(DataFormat::U8(&[PCD8544_SETYADDR | (page as u8)]))
                .unwrap();

            self.display_interface
                .send_commands(DataFormat::U8(&[PCD8544_SETXADDR]))
                .unwrap();

            self.display_interface
                .send_data(DataFormat::U8(
                    &self.buffer[(LCDWIDTH * page)..(LCDWIDTH * (page + 1))],
                ))
                .unwrap();
        }
        self.display_interface
            .send_commands(DataFormat::U8(&[PCD8544_SETYADDR]))
            .unwrap();
    }
}
    ...
    RST: OutputPin<Error = PinE>,
{
        ...
        I: IntoIterator<Item = Pixel<Self::Color>>,
    {
        for Pixel(coord, color) in pixels.into_iter() {
            ...
            if color.is_on() {
                self.buffer[coord.x as usize + ((coord.y as usize) / 8) * LCDWIDTH] |=
